...
(async () => {
  ...
  for (const site of todayMeta.sites) {
    const r = { id: site.id, name: site.name, url: site.url, importance: site.importance,
      ...
      today: { full: site.full, region: site.region }, prev: null, diff: null, errorMessage: null };
    try {
      const prevSite = prevMeta ? prevMeta.sites.find(s => s.id === site.id) : null;
      if (site.error) { r.status = 'error'; r.errorMessage = site.error; }
      else if (!prevSite || prevSite.error || !prevSite.full) { r.status = 'first'; }
      else {
        r.prev = { full: prevSite.full, region: prevSite.region };
        const useRegion = !!(site.region && prevSite.region);
        r.basis = useRegion ? 'region' : 'full';
        const diffPath = path.join(DIFF_DIR, today, `${site.id}.png`);
        r.changeRate = compareImages(useRegion ? prevSite.region : prevSite.full, useRegion ? site.region : site.full, diffPath);
